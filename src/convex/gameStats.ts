...
// Get top 3 players of the week across all games
export const getTopPlayersOfWeek = query({
  args: {},
  handler: async (ctx) => {
    const oneWeekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
    
    // Get all game sessions from the past week
    ...
    const playersWithDetails = await Promise.all(
      topPlayers.map(async (player) => {
        const user = await ctx.db.get(player.playerId as any);
        return {
          user,
          wins: player.wins,
        };
